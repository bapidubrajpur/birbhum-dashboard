...
# ----------------- DYNAMIC DATA DIRECTORY DETECTOR -----------------
# This checks if there is a local 'data' folder first (for running on your own computer)
# or falls back to the workspace container path.
if os.path.exists("./data"):
    DATA_DIR = "./data"
elif os.path.exists("data"):
    DATA_DIR = "data"
else:
    DATA_DIR = "/workspace/scratch/data"
...
def load_table_data(filename):
    ...
    if os.path.exists(filepath):
        ...
        st.error(f"Data file {filename} not found in directory!")
        return None
...
if role == "👥 Viewer (Reports Only)":
    ...
    with tab6:
        st.markdown("### 🔑 DSC Enrolment and De-enrolment Status under VB-GRAM G")
        st.caption("Tracking Digital Signature Certificate (DSC) registration and XML verification as on 22.08.2026")
        
        if df_dsc is not None:
            ds1, ds2, ds3, ds4 = st.columns(4)
            ds1.metric("Total DSC Enrolled", f"{df_dsc['No. of DSC Enrolled'].sum():,}")
            ds2.metric("Target DSC de-enrolment", f"{df_dsc['Target of DSC de-enrolment'].sum():,}")
            ds3.metric("De-enrolled DSC", f"{df_dsc['No. of DSC De-enrolled'].sum():,}")
            ds4.metric("XML Generated & Signed", f"{df_dsc['No. of XML generated and signed'].sum():,}")
            
            # Interactive Chart: DSC Enrolled vs Target
            st.markdown("#### Blockwise DSC Enrolled against Target")
            fig_dsc = px.bar(
                df_dsc,
                x="Block Name",
                y=["Target of DSC enrolment", "No. of DSC Enrolled"],
                barmode="group",
                color_discrete_sequence=["#FFC107", "#2E7D32"],
                labels={"value": "Count", "variable": "Type"}
            )
            fig_dsc.update_layout(xaxis_tickangle=-45)
            st.plotly_chart(fig_dsc, use_container_width=True)
            
            # Chart: XML signed %
            st.markdown("#### XML Signing Pending % by Block")
            fig_xml = px.bar(
                df_dsc,
                x="Block Name",
                y="% of Pending XML signing",
                color="% of Pending XML signing",
                color_continuous_scale=px.colors.sequential.YlOrRd,
                labels={"% of Pending XML signing": "Pending XML Signing (%)"}
            )
            fig_xml.update_layout(xaxis_tickangle=-45)
            st.plotly_chart(fig_xml, use_container_width=True)
            
            # Data table
            st.markdown("#### DSC Status Detailed Table")
            search_query6 = st.text_input("🔍 Search block in DSC table:", "")
            filtered_df6 = df_dsc[df_dsc['Block Name'].str.contains(search_query6, case=False)] if search_query6 else df_dsc
            st.dataframe(filtered_df6, use_container_width=True)
            
            # Download button
            csv6 = filtered_df6.to_csv(index=False).encode('utf-8')
            st.download_button("📥 Download DSC report as CSV", csv6, "dsc_enrolment_report.csv", "text/csv")
